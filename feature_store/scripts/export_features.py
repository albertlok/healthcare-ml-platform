...
def export_patient_demographics(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    df["age_at_appointment"] = (
        ((df["feature_timestamp"].dt.tz_localize(None) - df["date_of_birth"]).dt.days // 365)
        .clip(0, 120)
        .astype("int32")
    )
    df["gender_encoded"] = _encode(df["gender"], GENDER_MAP)
    df["insurance_type_encoded"] = _encode(df["insurance_type"], INSURANCE_MAP)
    df["preferred_communication_encoded"] = _encode(df["preferred_communication_channel"], COMM_MAP)
...
def export_appointment_context(conn: duckdb.DuckDBPyConnection) -> None:
    df = conn.execute(
        "SELECT * FROM main_staging.stg_appointments"
        " WHERE event_type IN ('SCHEDULED','RESCHEDULED')"
    ).df()
...
def export_provider_stats(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    df["provider_avg_appointment_duration"] = df["provider_avg_appointment_duration"].astype(
        "float32"
    )
